...
public class BuildScript
{
	...
	public static void BuildAssetBundles(bool force)
	{
		...
		BuildPipeline.BuildAssetBundles (outputPath, force ? BuildAssetBundleOptions.ForceRebuildAssetBundle : 0, EditorUserBuildSettings.activeBuildTarget);        
    }

    public static void BuildAssetBundlesForPlatform(BuildTarget target)
    {
        string outputPath = Path.Combine(kAssetBundlesOutputPath, BaseLoader.GetPlatformFolderForAssetBundles(target));
        if (!Directory.Exists(outputPath))
            Directory.CreateDirectory(outputPath);
